...
object ScalaBasics {
   ...
   */
  def oddRange(n: Int): Range = 1 to(2*n, 2)
  ...
  def minWhile(r: Array[Int]): Int = {
    ...
    var current = 0
    while (current < r.length) {
      if (r(current) < currentMin){
        currentMin = r(current)
      }
      current += 1
    }
    currentMin
  ...
  def minFor(r: Array[Int]): Int = {
    ...
    for (i <- 0 until r.length) {
      currentMin = Math.min(r(i), currentMin)
    }
    currentMin
  }
  ...
  def minRecursive(r: Array[Int]): Int = {
    ...
    else minRecursive(Array(Math.min(r(0), r(1))) union r.slice(2, r.length))
  }
  ...
  def isPalindrome(s: String): Boolean = {
    val cleanedString = (for {
      c <- s
      if c.isLetter
    } yield c.toLower).mkString
    cleanedString equals cleanedString.reverse
  }
  ...
  def wordCounter(lines: Array[String]): Map[String, Int] =
    lines.flatMap(line => line.split(" ")).groupBy(identity).mapValues(_.length)
