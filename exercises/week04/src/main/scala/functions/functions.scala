...
object Funcs {
  ...
  def drop[A](ls: List[A], n: Int): List[A] = ls match {
    case Nil => Nil
    case head :: tail => if (n <= 0) head :: tail else drop(tail, n-1)
  }
  ...
  def length[A](ls: List[A]): Int = foldLeft(ls, 0)((count, _) => count + 1)
  ...
  /**
    ...
    *         elements of ls.
    */
  def flatMap[A, B](ls: List[A])(f: A => List[B]): List[B] = flatten(map(ls)(f))
  ...
  def maxAverage(ls: List[(Double, Double)]): Double = {
    val maxList = map(ls)((x) => Math.max(x._1,x._2))
